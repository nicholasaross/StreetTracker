"""Re-soak check: max_concurrent=3 + direction-aware cadence.

Compare per-vehicle snap counts across:
- Step 11 anchor (max_concurrent=2, uniform 400ms, 5.5h daytime)
- Step 13a soak (max_concurrent=2, dir-aware 300/400ms, 18.4h)
- Re-soak live  (max_concurrent=3, dir-aware 300/400ms, partial)

Headline question: did raising max_concurrent finally let the direction-
aware cadence bind?
"""
...
def _summarize(label: str, records: list[dict]) -> None:
    ...
    print(
        ...
        f"at_cap_15={sum(1 for n in snaps if n >= 15)}"
    )
    by_dir: dict[str, list[int]] = defaultdict(list)
    durs_by_dir: dict[str, list[float]] = defaultdict(list)
    for r in vehicles:
        by_dir[r["direction"]].append(len(r.get("main_snaps") or []))
        durs_by_dir[r["direction"]].append(r["duration_visible"])
    for d in sorted(by_dir):
        ...
        print(
            f"    {d:18s}: n={len(ns):4d}  "
            f"snaps mean={statistics.mean(ns):.2f}  median={statistics.median(ns)}  "
            f"snaps/sec mean={statistics.mean(rates):.3f}  "
            f"dur median={statistics.median(durs):.2f}s  "
...
def main() -> int:
    ...
    _summarize(
        "Re-soak live (max_concurrent=3, dir-aware 300/400ms)",
        _load(Path(".claude/resoak_events.jsonl")),
    )
    return 0
